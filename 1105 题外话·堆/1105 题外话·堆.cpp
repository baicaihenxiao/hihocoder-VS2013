...
在一组测试数据中�?
�?行为1个整数N，表示需要处理的事件数目�?
接下来的M行，每行描述一个事件，且事件类型由该行的第一个字符表示，如果�?A'，表示小Ho将一粒糖果放进了盒子，且接下来为一个整数W，表示这颗糖果的重量；如果为'T'，表示小Ho需要知道当前盒子中最重的糖果的重量是多少，在知道这个值之后，小Ho会将这颗糖果从盒子中取出并吃掉�?
...
在一组测试数据中�?
对于每个类型�?T'的时间，输出1个整数W_MAX，表示在这一时刻，盒子中最重的糖果的重量�?
